...
//    List<Ticket> getTicketsBoughtByUser(Long userId);
//
////Write a JPQL query that returns all tickets between a range of dates
//    @Query("SELECT t FROM Ticket t WHERE t.dateTime BETWEEN ?1 AND ?2")
//    List<Ticket> getTicketsByDate(LocalDateTime start, LocalDateTime end);
//
//// ------------------- Native QUERIES ------------------- //
////Write a native query to count the number of tickets a user bought
//    @Query(value = "SELECT COUNT(*) FROM ticket WHERE  user_account_id = ?1", nativeQuery = true)
//    Integer countTicketsByUser(Long userId);
//
...
//    Integer countTicketsByUserInDateRanges(Long useId, LocalDateTime start, LocalDateTime end);
//
////Write a native query to distinct all tickets by movie name
//    @Query(value = "SELECT  DISTINCT(m.name) FROM ticket JOIN movie_cinema mc ON mc.id=ticket.movie_cinema JOIN movie m ON mc.movie_id=m_id ", nativeQuery = true)
//    List<String> retrieveAllDistictMovieName();
//
...
//    List<Ticket> getAllUserByEmail(String email);
//
...
//            "JOIN move_cinema mc ON ticket.movie_cinema_id=mc.id" +
//            "JOIN movie m ON mc.movie_id =m.id" +
//            "WHERE ua.username ILIKE concat('%', ?1 '%') OR ad.name ILIKE concat('%', ?1 '%') OR m.name ILIKE concat('%', ?1 '%') ",
//            nativeQuery = true)
//    List<Ticket> getAllTicketByUserNameAndMovieName(String searchCriteria);
